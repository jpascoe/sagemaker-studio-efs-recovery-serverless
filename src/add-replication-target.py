...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-src-domain-name", "--src-domain-name", dest="src_domain_name", type=str
    )
    parser.add_argument(
        "-target-domain-name",
        "--target-domain-name",
    ...
    parser.add_argument(
        "-src-profile-name", "--src-profile-name", dest="src_profile_name", type=str
    )
    parser.add_argument(
        "-target-profile-name",
        "--target-profile-name",
    ...
    parser.add_argument(
        "-table-name", "--table-name", dest="table_name", type=str, default="studioUser"
    )
    parser.add_argument(
        "-history-table-name",
        "--history-table-name",
    ...
    if item_target:
        sys.exit(
            f"target domain_name {target_domain_name}, target profile_name {target_profile_name} already exists. Try different profile."
        )
    ## target doesn't exist yet. replication takes place on subsequent create profile event.
    print("-" * 50)
    print(
        f"append table {table} with new domain {target_domain_name} and profile names {target_profile_name}"
    )
    ...
    print("-" * 50)
    users_hist = hist.UsersHistory(ddb_resource=ddb_resource, table_name=history_table)
