...
def lambda_handler(event, context):
    ...
    user_profile_name = event.get("requestParameters").get("userProfileName")
    single_sign_on_user_value = event.get("requestParameters").get(
    ...
    if user_profile_name:
        # role_name = ""
        logger.info(
            f"Create UserProfile event detected with domain {domain_id} userprofile {user_profile_name}"
        )
        profile = p.Profile(
            domain_id=domain_id,
        ...
        # role_name = ""
        logger.info(
            f"Create Space event detected with domain {domain_id} space {space_name}"
        )
        profile = p.Profile(
            domain_id=domain_id,
            sm_client=boto3.client("sagemaker"),
        ...
        logger.info(
            f"""Built profile: space {profile.space}\
        role_name {profile.role}\
        domain_id {profile.domain_id}\
        home_efs_id {profile.efs_sys_id}\
        efs_uid {profile.efs_uid}"""
    ...
    response = users.update_user(
        key={
            os.getenv("HASHKEY", "profile_name"): profile.name
            + profile.space,  # either one is empty string,
            os.getenv("RANGEKEY", "domain_name"): profile.domain_name,
        },
        expression="set replication = :rp, domain_id = :d, profile_name = :pn, user_profile_name = :upn, space_name = :s, efs_sys_id =:es, efs_uid = :eu, role_name =:rn",
        attributes={
            ":rp": True,
            ":rn": profile.role,
            ":d": domain_id,
            ":pn": profile.name,
            ":upn": profile.user_profile_name,
            ":s": profile.space,
    ...
    users_hist.put_user(
        item={
            os.getenv("HASHKEY_HIST", "profile_name"): profile.name
            + profile.space,  # either one is empty string
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-user_profile_name", "--user_profile_name", dest="user_profile_name", type=str
    )
    ...
    event = {
        ...
        "detail": {
            ...
            "requestParameters": {
                ...
                "userSettings": {"executionRole": args.role_name},
            },
        },
    }
