...
def start_execution(arn: string, input: string, client) -> Mapping:
    try:
        response = client.start_execution(stateMachineArn=arn, input=input)
    except ClientError as e:
        logger.error(
...
def lambda_handler(event, context):
    ...
    if not (replication_flag):
        logger.info(
            f"replication flag set to {replication_flag} for profile {profile_name}. skip replication"
        )
        return
    ...
    if profile.error:
        ...
        """
        logging.warning(
            f"could not get user profile metadata. skip user {profile_name}"
        )
        return

    logger.info(
        f"""Built profile: user {profile.name}\
    ...
    for a in names:
        if not (os.getenv(a)):
            raise ValueError(
                f"invalid os env parameter. key {a} does not exist or value is empty"
            )
    ...
    source_user_profile_name = event["dynamodb"]["OldImage"]["user_profile_name"].get(
        "S"
    )
    ...
    if (source_efs == target_efs) and (source_mount == target_mount):
        logging.warning(
            f"Source EFS Volume and Directory UID cannot be the same. skip profile {profile_name}"
        )
        return
    input = {
        "Options": options,
        ...
        "Source": {
            ...
            "SecurityGroupArns": [
                f"arn:aws:ec2:{region}:{account}:security-group/{sg}"
                for sg in params["SOURCE_SECURITY_GROUP"].split(",")
                if sg
            ],
        },
        "Target": {
            ...
            "SecurityGroupArns": [
                f"arn:aws:ec2:{region}:{account}:security-group/{sg}"
                for sg in params["TARGET_SECURITY_GROUP"].split(",")
                if sg
            ],
        },
    }
    if space_name:
        ...
        input["Source"]["ProfileName"] = source_profile_name
        input["Target"]["ProfileName"] = profile_name
    step_function_name = params["STEPFUNCTION"].rsplit(":")[-1]
    logger.info(
        f"invoke stepfunction {params['STEPFUNCTION'].rsplit(':')[-1]} with input {input}"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("-domain-id", "--domain-id", dest="domain_id", type=str)
    parser.add_argument(
        "-profile-name", "--profile-name", dest="profile_name", type=str
    )
    parser.add_argument("-role-name", "--role-name", dest="role_name", type=str)
    parser.add_argument("-region", "--region", dest="region", type=str)
    parser.add_argument("-efs-sys-id", "--efs-sys-id", dest="efs_sys_id", type=str)
