...
def list_profiles(domain_id: string, client) -> List[str]:
    ...
    try:
        ...
        page_iterator = paginator.paginate(
            DomainIdEquals=domain_id, PaginationConfig={"MaxItems": 500}
        )
...
def get_all_users_metadata(
    domain_id: string, efs_id: string, profile_names: List[str], client
):
    data = []
    for name in profile_names:
        ...
        if response:
            ...
            if "SingleSignOnUserValue" in response:
                user_meta["SingleSignOnUserValue"] = response["SingleSignOnUserValue"]
            if (
                "UserSettings" in response
                and "ExecutionRole" in response["UserSettings"]
            ):
                user_meta["ExecutionRole"] = response["UserSettings"][
                    "ExecutionRole"
                ].rsplit("role/")[-1]
...
def get_all_spaces_metadata(
    domain_id: string, efs_id: string, space_names: List[str], client
):
    data = []
...
def lambda_handler(event, context):
    ...
    try:
        ...
        domain_ids = get_domain_ids(
            sagemaker
        )  # event.get('ResourceProperties')['DOMAIN_ID']
        ...
        for domain_id in domain_ids:
            ...
            efs_id = domain_metadata[
                "HomeEfsFileSystemId"
            ]  # event.get('ResourceProperties')['EFS_ID']
            ...
            with table.batch_writer() as batch:
                ...
                for record in data:
                    item = {
                        os.getenv("HASHKEY_HIST"): record.get(
                            "SingleSignOnUserValue", ""
                        )
                        + record.get(
                            "SpaceName", ""
                        ),  # either UserProfileName or SpaceName is empty string
                        "replication": True,
