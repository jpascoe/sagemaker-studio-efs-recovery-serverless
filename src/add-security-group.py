...
def update_security_groups(source: string, target: string, client):
    try:
        ...
        response = client.authorize_security_group_ingress(
            GroupId=target, IpPermissions=ip_permission
        )
...
def lambda_handler(event, context):
    ...
    try:
        ...
        logger.info(
            f"obtained security groups {security_groups} from target_mount_id {mount_id}"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-region", "--region", dest="region", type=str, default="us-east-2"
    )
    parser.add_argument("-efs-id", "--efs-id", dest="efs_id", type=str)
    parser.add_argument(
        "-security-groups", "--security-groups", dest="security_groups", type=str
    )
