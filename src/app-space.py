    ...
    try:
        ...
        while not is_created:
            ...
            if response["Status"] == "InService":
                ...
                logging.error(
                    f"create app {space_name} in pending state. proceeding: {response}"
                )
                return
    except:
...
def delete_apps_space(domain_id, space_name):
    ...
    for p in sm_client.get_paginator("list_apps").paginate(
        ...
        for a in p["Apps"]:
            if (
                a["AppType"] in ["KernelGateway", "JupyterServer"]
...
def lambda_handler(event, context):
    ...
    try:
        if event["RequestType"] == "Create":
            ...
            delete_apps_space(domain_id=domain_id, space_name=space_name)
        cfnresponse.send(
            event,
            context,
            cfnresponse.SUCCESS,
